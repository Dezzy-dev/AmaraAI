...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      if (!userData) {
        console.log('No userData available, skipping data load');
        return;
      }
      ...
      try {
        ...
        // Load recent mood logs with error handling
        ...
        try {
          ...
          console.log('Loaded mood logs:', moodLogs.length);
        } catch (moodError) {
          console.error('Error loading mood logs:', moodError);
          setRecentMoods([]);
        }

        // Load recent journal entries with error handling
        ...
        setDataLoadError('Failed to load dashboard data. Please refresh the page.');
      } finally {
        ...
        <div className="bg-red-50 dark:bg-red-900/20 border-b border-red-200 dark:border-red-700">
          <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 xl:px-8 py-3">
            <div className="flex items-center space-x-3">
              ...
              <p className="text-red-800 dark:text-red-200 text-sm">{dataLoadError}</p>
              <button
                onClick={() => window.location.reload()}
                className="ml-auto px-3 py-1 bg-red-600 text-white text-xs rounded hover:bg-red-700 transition-colors"
              >
                Refresh
              </button>
            </div>
